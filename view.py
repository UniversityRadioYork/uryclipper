...
def getAudioBetween(start,end,location):
    ...
    start_file = 0
    end_file = 0
    for i in os.listdir(config.log_location+date_dir):
    ...
    if start_file == end_file:
        ...
        clip = AudioSegment.from_mp3(config.log_location+date_dir+"/"+str(audio_file)+".mp3")
        clip = clip[(start_time*1000):(end_time*1000)]
    else:
        clip = AudioSegment.from_mp3(config.log_location+date_dir+"/"+str(files[start_file])+".mp3")
        clip = clip[(start-files[start_file])*1000:]
        for i in files[start_file+1:end_file]:
            section = AudioSegment.from_mp3(config.log_location+date_dir+"/"+str(i)+".mp3")
            clip = clip + section
        end_clip = AudioSegment.from_mp3(config.log_location+date_dir+"/"+str(files[end_file])+".mp3")
...
def index():
    if verifySession(session):
        ...
        return redirect("https://ury.org.uk/myradio/MyRadio/jwt?redirectto="+config.clipper_url+"auth/", code=302)

@app.route('/makeaudio/<key>/<start>/<end>')
def makeaudio(key,start,end):
    if verifyKey(key) and verifySession(session):
        return getAudioBetween(int(start),int(end),config.clips_location+str(key)+"cl.mp3")

@app.route('/getaudio/<key>/')
def getaudio(key):
    if verifyKey(key) and verifySession(session):
        return send_file(config.clips_location+str(key)+"cl.mp3")

@app.route('/makeclip/<key>/<start>/<end>')
def makeclip(key,start,end):
    if verifyKey(key) and verifySession(session):
        clip = AudioSegment.from_mp3(config.clips_location+str(key)+"cl.mp3")
        clip = clip[(int(start)*1000):(int(end)*1000)]
        clip.export(config.clips_location+str(key)+"clip.mp3", format="mp3")
        return {"status": "complete"}

@app.route('/getclip/<key>/')
def getclip(key):
    if verifyKey(key) and verifySession(session):
        return send_file(config.clips_location+str(key)+"clip.mp3")

@app.route('/auth/', methods=['GET'])
def auth( ):
    args = request.args
    userinfo = jwt.decode(args['jwt'], config.myradio_key, algorithms=["HS256"])
    print(userinfo)
    session['name'] = userinfo['name']
    session['uid'] = userinfo['uid']
    return redirect(config.clipper_url, code=302)
